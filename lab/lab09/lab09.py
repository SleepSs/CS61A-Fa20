...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        perms = subseqs(s[1:])
        # print(perms)
        return insert_into_all(s[0],perms) + perms           
...
def make_generators_generator(g):
    ...
    def gen(i):
        for x in g():
            ...
            i -= 1
    i = 1
    for _ in g(): #限定总yield次数
        yield gen(i) #yield上加yield
        i += 1
...
def make_advanced_counter_maker():
    ...
    gcount = 0 #Everytime call MACM gcount reset, so be careful where gcount shoubld be initialized
    def big():
        count = 0
        def kids(gut):
            ...
            if gut == 'count':
                count += 1
                return count
            elif gut == 'global-count':
                gcount += 1
                return gcount
            elif gut == 'reset':
                count = 0
            elif gut == 'global-reset':
                gcount = 0
        return kids
    return big
