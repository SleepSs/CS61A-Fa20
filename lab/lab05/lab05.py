...
def closer_city(lat, lon, city_a, city_b):
    ...
    a_dis = distance(temp_city, city_a)
    b_dis = distance(temp_city, city_b)
    return get_name(city_a) if a_dis < b_dis else get_name(city_b)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[int((i + (i % 2) * len(deck))/2)] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        print("DEBUG:",t1,"sep",t2)
        return tree(label(t1) + label(t2), add_trees(branches(t1),branches(t2)))
