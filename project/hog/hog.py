...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    if not (player_score and opponent_score):
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    "*** YOUR CODE HERE ***"
    def win(score0, score1):
        if score0 >= goal or score1 >= goal:
            return True

    while not win(score0, score1):
        if who == 0:
            rolls = strategy0(score0, score1)
            adding = take_turn(rolls, score1, dice)
            score0 += adding
            say = say(score0, score1)
            if win(score0, score1):
        ...
        if who == 1:
            ...
            adding = take_turn(rolls, score0, dice)
            score1 += adding
            say = say(score0, score1)
            if win(score0, score1):
                return score0, score1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            addon = score0 - last_score
            pr = running_high
            if addon > pr:
                print(addon, "point(s)! The most yet for Player 0")
                pr = addon
            return announce_highest(0, score0, pr)
        
        if who == 1:
            addon = score1 - last_score
            pr = running_high
            if addon > pr:
                print(addon, "point(s)! The most yet for Player 1")
                pr = addon
            return announce_highest(1, score1, pr)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    how_much_rolled = 1
    for i in range(1,11):
        testdice = make_averaged(roll_dice, trials_count)
        temp = testdice(i, dice)
        if highest < temp:
            highest = temp
            how_much_rolled = i
    return how_much_rolled
