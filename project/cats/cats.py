...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    answer_sheet = [x for x in paragraphs if select(x) == True]
    if len(answer_sheet) - 1 >= k:
        return answer_sheet[k]
    else:
        return ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if typed == "":
        return 0.0
    while index <= len(reference_words) - 1 and index <= len(typed_words) - 1:
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    score_list = [diff_function(user_word, x, limit) for x in valid_words]
    least = min(score_list)
    if least > limit:
        return user_word
...
def report_progress(typed, prompt, user_id, send):
    ...
    "*** YOUR CODE HERE ***"
    match, index = 0, 0
    while index <= len(typed) - 1:
        if typed[index] == prompt[index]:
            match, index = match + 1, index + 1
        else:
            break
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    timee = times_per_player
    for x in range(len(times_per_player)):
        for i in range(len(times_per_player[x]) - 1):
            timee[x][i] = times_per_player[x][i+1] - times_per_player[x][i]
        timee[x].pop(-1)
    
    return game(words, timee)
    # END PROBLEM 9
    ...
    for i in word_indices:
        ...
        for p in player_indices:
            ...
            if pt < min_t:
                min_t = pt
                winp = p
        answer_sheet[winp].append(word_at(game, i))
    return answer_sheet
    # END PROBLEM 10
