...
def eval_all(expressions, env):
    ...
    2
    """
    # Begin挺神奇的，刚开始以为什么用没有，现在发现只要是函数调用都得用到它。
    result = None
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        v = scheme_eval(expressions.rest.first, env) # 不加first就挂
        env.define(target, v)
        return target
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    try:
        ...
        raise SchemeError("Invalid text")
    if expressions.rest is nil:
        return is_true_primitive(expressions.first) and evaled
    elif is_true_primitive(evaled):
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        nuts = bindings.first
        validate_form(nuts, 2, 2)
        names, values = Pair(nuts.first, names), Pair(scheme_eval(nuts.rest.first, env), values)
        bindings = bindings.rest
    validate_formals(names) # 放在最后而且仅eval NAME 若eval bindings 只要是数字则会报错
    # END PROBLEM 14
