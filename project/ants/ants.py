...
"""Summary:Always check whether an attribute is for class or instances, if for instances, then use init to create it."""
################
# Core Classes #
################
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while p is not beehive:
            if p.bees and self.min_range <= dis <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount < self.armor:
            for b in self.place.bees[:]: # 为什么能边iterate边删除另一个list， 这两个对象是链接到同一个吗？
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.place.bees and not self.digesting:
...
# END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_armor(amount)
        if not self.fake and self.armor <= 0:
            bees_win()
        # END Problem EC
